...
export default async function Page() {
  let dataSettings, dataHome;
  try {
    dataSettings = await fetcher(`${process.env.NEXT_PUBLIC_API_BASE_URL}/settings`);
    dataHome = await fetcher(`${process.env.NEXT_PUBLIC_API_BASE_URL}/home`);
  } catch (error) {
    return <div>Error loading data</div>;
  }

  if (!dataSettings || !dataHome) {
    return <div><Loading /></div>;
  }

  const seoTitle = dataHome?.seo_title || "";
  const seoDesc = dataHome?.seo_description || "";
